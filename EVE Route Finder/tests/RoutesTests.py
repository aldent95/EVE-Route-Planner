...
##    def test_Positive_get_Route_Jumps_Safe_Waypoints(self):
##        waypoints =[self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']],self.systems[self.sysNames['Hek']]]
##        self.testRoute = RouteFinder(self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']], self.systems, [],waypoints=waypoints)
##        expectedResult = 29
##        actualResult = self.testRoute.getRoute('safe')
##        self.assertEqual(expectedResult, len(actualResult)-1, "Fail: Route length not correct, must be a fault somewhere in route generation")
    def test_Positive_get_Route_Jumps_LessSafe_Waypoints(self):
        waypoints =[self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']],self.systems[self.sysNames['Hek']]]
        self.testRoute = RouteFinder(self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']], self.systems, [],waypoints=waypoints)
        expectedResult = 79
        actualResult = self.testRoute.getRoute('lessSafe')
        ...
        printRoute(actualResult)
        self.assertEqual(expectedResult, len(actualResult)-1, "Fail: Route length not correct, must be a fault somewhere in route generation")
##    def test_Positive_get_Route_Jumps_Waypoints(self):
##        waypoints =[self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']],self.systems[self.sysNames['Hek']]]
##        self.testRoute = RouteFinder(self.systems[self.sysNames["Jita"]], self.systems[self.sysNames['Amarr']], self.systems, [], waypoints=waypoints)
##        expectedResult = 24
##        actualResult = self.testRoute.getRoute('normal')
##        self.assertEqual(expectedResult, len(actualResult)-1, "Fail: Route length not correct, must be a fault somewhere in route generation")
##
